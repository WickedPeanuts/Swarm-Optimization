...
namespace Particle_Swarm_Optimization
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //BeginSimulationFFA(EFunction.Rosenbrock, i++);
            //BeginSimulationFFA(EFunction.RotatedRastrigin, i++);

            #endregion FFA

            #region ABC

            BeginSimulationABC(EFunction.Sphere, i++);
            BeginSimulationABC(EFunction.Rosenbrock, i++);
            BeginSimulationABC(EFunction.RotatedRastrigin, i++);
        ...
        public static void BeginSimulationABC(EFunction fun, int id)
        {
            List<List<Double>> lld = new List<List<Double>>();
            for (int i = 0; i < Parameters.SAMPLE_COUNT; i++)
            {
                Artificial_Bee_Colony.Swarm s = (new Artificial_Bee_Colony.Swarm(fun));
                s.InitializeSwarm();
                ...
                Console.WriteLine("Iteração " + i + ": GBest: " + FireflyParticle.GlobalBestFitness);
            }

            SaveToFile(ETopology.Global, fun, EConstrictionFactor.FixedInertia, lld, id);
